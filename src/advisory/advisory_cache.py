...
class AdvisoryCache:
    ...
    def count_advisories_with_attribute(self, attr_name: str, protocol_version: str = "1.0", stage: str = "ic") -> int:
        """Count cached advisories where getattr(advisory, attr_name) is truthy.
        
        Iterates all cached keys for the given protocol+stage and counts
        how many advisories have the given attribute set to a truthy value.
        Much more efficient than per-article lookups from the UI.
        """
        count = 0
        keys = self.list_cached(protocol_version)
        for key in keys:
            try:
                advisory = self.get(key, protocol_version, stage)
                if advisory and getattr(advisory, attr_name, False):
                    count += 1
            except Exception:
                pass
        return count

    def count_prefiltered(self, protocol_version: str = "1.0", stage: str = "ic") -> int:
        return self.count_advisories_with_attribute("prefilter_applied", protocol_version, stage)

    def count_quarantined(self, protocol_version: str = "1.0", stage: str = "ic") -> int:
        count = 0
        keys = self.list_cached(protocol_version)
        for key in keys:
            try:
                advisory = self.get(key, protocol_version, stage)
                stage_val = getattr(advisory, 'stage_validation', "") or ""
                if advisory and "QUARANTINED" in str(stage_val):
                    count += 1
            except Exception:
                pass
        return count

    def count_llm_generated(self, protocol_version: str = "1.0", stage: str = "ic") -> int:
        """Count advisories generated by LLM (not prefiltered) that are available."""
        count = 0
        keys = self.list_cached(protocol_version)
        for key in keys:
            try:
                advisory = self.get(key, protocol_version, stage)
                if advisory and not getattr(advisory, 'prefilter_applied', False) and advisory.is_available():
                    count += 1
            except Exception:
                pass
        return count

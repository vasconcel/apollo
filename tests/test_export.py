...
class TestFileAndSheets:
    ...
    def test_no_ai_columns(self, mock_paper_repo, mock_decision_repo, tmp_path):
        ...
        for sheet_name in wb.sheetnames:
            ws = wb[sheet_name]
            headers = [str(c.value or "") for c in (ws[1] if sheet_name == "White Literature" else ws[2])]
...
class TestGLSheet:
    def test_double_header_layout(self, mock_paper_repo, mock_decision_repo, tmp_path):
        from src.use_cases.export_papers import ExportScreenedPapersUseCase
        ...
        headers_row1 = [c.value for c in ws[1]]
        headers_row2 = [c.value for c in ws[2]]
        assert headers_row1[0] == "#"
        assert headers_row1[1] == "Title"
        assert headers_row1[2] is None
        assert headers_row1[3] is None
        assert headers_row1[4] == "Decision"
        assert headers_row2[0] is None
        assert headers_row2[1] is None
        assert headers_row2[2] == "Revisor 1"
        assert headers_row2[3] == "Revisor 2"
        assert headers_row2[4] is None

    def test_merged_cells(self, mock_paper_repo, mock_decision_repo, tmp_path):
        from src.use_cases.export_papers import ExportScreenedPapersUseCase
        ...
        assert "A1:A2" in merged, f"Merged ranges: {merged}"
        assert "B1:B2" in merged, f"Merged ranges: {merged}"
        assert "E1:E2" in merged, f"Merged ranges: {merged}"
    ...
    def test_row_content(self, mock_paper_repo, mock_decision_repo, tmp_path):
        ...
        assert ws.cell(row=3, column=2).value == "Paper Two"

    def test_revisor1_numeric_format(self, mock_paper_repo, mock_decision_repo, tmp_path):
        from src.use_cases.export_papers import ExportScreenedPapersUseCase
    ...
    def test_revisor1_comment_exists(self, mock_paper_repo, mock_decision_repo, tmp_path):
        from src.use_cases.export_papers import ExportScreenedPapersUseCase
        ...
        wb = load_workbook(output)
        ws = wb["Grey Literature"]
        comment = ws["C2"].comment
        assert comment is not None
        assert "IC1" in comment.text
        assert "EC1" in comment.text

    def test_revisor2_comment_exists(self, mock_paper_repo, mock_decision_repo, tmp_path):
        from src.use_cases.export_papers import ExportScreenedPapersUseCase
...
class TestSequentialNumbering:
    def test_numbering_sequential(self, mocker: MockerFixture, tmp_path):
        ...
        gl_nums = [ws_gl.cell(row=r, column=1).value for r in range(3, ws_gl.max_row + 1)]
        assert gl_nums == [1]
...
class TestStyling:
    ...
    def test_gl_header_bold(self, mock_paper_repo, mock_decision_repo, tmp_path):
        ...
        wb = load_workbook(output)
        ws = wb["Grey Literature"]
        for cell in list(ws[1]) + list(ws[2]):
            if cell.value:
                assert cell.font.bold is True
...
class TestEmptyDecisions:
    def test_empty_creates_both_sheets_with_only_headers(self, mock_paper_repo, mocker: MockerFixture, tmp_path):
        ...
        gl_data = list(ws_gl.iter_rows(min_row=3, values_only=True))
